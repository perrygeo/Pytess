...
def triangulate(points):
    ...
    """
    # Remove duplicate xy points bc that would make delauney fail, and must
    # remember z (if any) for retrieving originals from index results
    seen = set()
    uniqpoints = [p for p in points if str(
        p[:2]) not in seen and not seen.add(str(p[:2]))]
    ...
    triangles = [[uniqpoints[i] for i in triangle]
...
def voronoi(points, buffer_percent=100):
    ...
    # For a similar approach and problem see:
    # http://gis.stackexchange.com/questions/11866/voronoi-polygons-that-run-out-to-infinity
    xs, ys = list(zip(*uniqpoints))[:2]
    pointswidth = max(xs) - min(xs)
    pointsheight = max(ys) - min(ys)
    xbuff, ybuff = (pointswidth / 100.0 * buffer_percent,
                    pointsheight / 100.0 * buffer_percent)
    midx, midy = (sum(xs) / float(len(xs)), sum(ys) / float(len(ys)))
    # bufferbox =
    # [(midx-xbuff,midy-ybuff),(midx+xbuff,midy-ybuff),(midx+xbuff,midy+ybuff),(midx-xbuff,midy+ybuff)]
    # # corner buffer
    bufferbox = [(midx - xbuff, midy), (midx + xbuff, midy),
                 (midx, midy + ybuff), (midx, midy - ybuff)]  # mid sides buffer
