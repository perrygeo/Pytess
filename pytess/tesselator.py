...
class Context(object):
    ...
    def __init__(self):
        ...
        # equation of line 3-tuple (a b c), for the equation of the line
        # a*x+b*y = c
        self.lines = []
        # edge 3-tuple: (line index, vertex 1 index, vertex 2 index)   if
        # either vertex index is -1, the edge extends to infiinity
        self.edges = []
    ...
    def outBisector(self, edge):
        ...
        if(self.debug):
            print("line(%d) %gx+%gy=%g, bisecting %d %d" % (edge.edgenum,
                                                            edge.a, edge.b, edge.c, edge.reg[0].sitenum, edge.reg[1].sitenum))
        elif(self.triangulate):
            if(self.plot):
